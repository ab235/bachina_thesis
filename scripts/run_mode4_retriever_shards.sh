...
echo "Starting mode4 retriever shards in parallel: sbert, e5, bm25s ..."
run_one "sbert" "${SBERT_OUT}" "${LOG_DIR}/mode4_shard_sbert.log" &
PID_SBERT=$!
run_one "e5" "${E5_OUT}" "${LOG_DIR}/mode4_shard_e5.log" &
PID_E5=$!
run_one "bm25s" "${BM25S_OUT}" "${LOG_DIR}/mode4_shard_bm25s.log" &
PID_BM25S=$!
