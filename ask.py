import argparse
from textwrap import dedent  # Removes common leading whitespace from multi-line strings
...
def build_prompt(question: str, contexts: List[str]) -> tuple[str, str]:
    ...
    user = dedent(f"""\
    Question: {question}
    Context (top matches from the handbook): {joined}
    Reply with the best possible answer grounded in the context. If unsure, say so.
    """)
    return system, user

def format_contexts(retrieved: List[tuple]) -> List[str]:
    contexts = []
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="Example: python ask.py 'What is the refund policy?'"
    )
    parser.add_argument(
        "question", 
        type=str, 
        help="Your question about the handbook"
    )
